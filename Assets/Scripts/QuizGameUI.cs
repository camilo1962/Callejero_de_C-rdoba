...
#pragma warning restore 649
    ...
    /// <summary>
    ///Método que completa la pregunta en la pantalla
    /// </summary>
    ...
    public void SetQuestion(Question question)
    {
        ...
        //sofoca la lista de opciones
        List<string> ansOptions = ShuffleList.ShuffleListItems<string>(question.options);

        //asignar opciones a los respectivos botones de opción
        for (int i = 0; i < options.Count; i++)
        {
            //set the child text
            options[i].GetComponentInChildren<Text>().text = ansOptions[i];
            options[i].name = ansOptions[i];    //establecer el nombre del botón
    ...
    void OnClick(Button btn)
    {
        if (quizManager.GameStatus == GameStatus.PLAYING)
        {
            ...
            if (!answered)
            {
                //conjunto respondió verdadero
                answered = true;
                //obtener el valor bool
    ...
    //esto da un efecto de parpadeo [si es necesario, use o no use]
